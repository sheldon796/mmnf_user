<?php include("config.php");  ?>

<!DOCTYPE html>


<html lang="en-US">

<head>

<meta charset="UTF-8">

<meta name="description" content="">

<meta name="keywords" content="HTML,CSS,XML,JavaScript">

<meta name="author" content="Car Dealer Template">

<meta name="viewport" content="width=device-width, initial-scale=1.0">

<title>MMNF</title>

<link href='http://fonts.googleapis.com/css?family=Roboto:400,300,500,700' rel='stylesheet' type='text/css'>

<link rel="stylesheet" href="assets/css/bootstrap.css">

<link rel="stylesheet" href="assets/css/font-awesome.min.css">

<link rel="stylesheet" href="assets/css/main.css">

<!-- Slider Pro Css -->

<link rel="stylesheet" href="assets/css/sliderPro.css">

<!-- Owl Carousel Css -->

<link rel="stylesheet" href="assets/css/owl-carousel.css">

<!-- Flat Icons Css -->

<link rel="stylesheet" href="assets/css/flaticon.css">

<!-- Animated Css -->

<link rel="stylesheet" href="assets/css/animated.css">

<!--[if lt IE 9]>

	<script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>

	<script src="https://oss.maxcdn.com/libs/respond.js/1.3.0/respond.min.js"></script>

	<![endif]-->

</head>

<body>

<header class="site-header wow fadeIn" data-wow-duration="1s">

  <div id="main-header" class="main-header">

    <div class="container clearfix">

      <div class="logo"> <a href="index.php"></a> </div>

      <div id='cssmenu'>

        <ul>

          <li><a href='index.php'>Home</a></li>

          <li><a href='about_us.php'>About</a></li>

          <li><a href='#'>Events</a>

            <ul>

              <!--<li><a href='#'>Sidebar</a>

					            	<ul>

					               		<li><a href='car_listing_sidebar.html'>Car Listing</a></li>

					               		<li><a href='car_grid_sidebar.html'>Car Grid</a></li>

					            	</ul>

					        	</li>

					         	<li><a href='#'>No Sidebar</a>

					            	<ul>

					               		<li><a href='car_listing_no_sidebar.html'>Car Listing</a></li>

					               		<li><a href='car_grid_no_sidebar.html'>Car Grid</a></li>

					            	</ul>

					         	</li>-->

              <li><a href="events.php">Events</a></li>

              <li><a href="acheivements.php">Acheivements</a></li>

              <li><a href="guestlectures.php">Guest Lectures</a></li>

            </ul>

          </li>

          <!--<li class='active'><a href='#'>Blog</a>

					      	<ul>

					         	<li><a href='#'>Sidebar</a>

					            	<ul>

					               		<li><a href='blog_listing_sidebar.html'>Blog Classic</a></li>

					               		<li><a href='blog_grid_sidebar.html'>Blog Grid</a></li>

					            	</ul>

					         	</li>

					         	<li><a href='#'>No Sidebar</a>

					            	<ul>

					               		<li><a href='blog_listing_no_sidebar.html'>Blog Classic</a></li>

					               		<li><a href='blog_grid_no_sidebar.html'>Blog Grid</a></li>

					            	</ul>

					         	</li>

					         	<li><a href="single_post.html">Single Post</a></li>

					      	</ul>		

					   </li>-->

          <li><a href='#'>Departments</a>

            <ul>

              <li><a href="dept_mdc.php">Minority Development Centre (MDC) </a></li>

              <li><a href="dept_cdc.php">Community Development Centre (CDC)</a></li>

              <li><a href="dept_dpc.php">Data and Planning Centre (DPC)</a></li>

              <li><a href="dept_marti.php">Minority Affairs Research & training Institute (MARTI) </a></li>

              <li><a href="dept_ace.php">Academy for Competitive Exam (ACE)</a></li>

              <li><a href="dept_hrnl.php">Human Rights and Legal Forum</a></li>

              <li><a href="dept_sdf.php">Skill Developement Forum</a></li>

              <li><a href="dept_technobuiz.php">Technobuiz-Business School, Technical, Educational and Research Development Centre</a></li>

              <li><a href="dept_bdf.php">Business Development Forum</a></li>

              <li><a href="dept_healthcare.php">Healthcare and Paramedical Training Centre</a></li>

              <li><a href="dept_women_emp.php">Women Empowerment Centre</a></li>

              <li><a href="dept_financial_dev.php">Financial Development Centre</a></li>

              <li><a href="dept_travel.php">Travel and Guidance Centre</a></li>

              <li><a href="dept_disaster_mg.php">Disaster Management Centre</a></li>

              <li><a href="dept_waqi_dev.php">Waqf Development Forum </a></li>

            </ul>

          </li>

          

          <li><a href='#'>Gallery</a>

          <ul>

           <li><a href="photogallery.php">Photo Gallery</a></li>

           <li><a href="videogallery.php">Video Gallery</a></li>

           <li><a href="newsmedia.php">News Media</a></li>

          </ul>

             </li><li><a href='#'>Registration</a>

            <ul>

              <li><a href="ref_individual.php">Individual</a></li>

              <li><a href="ref_NGO.php">NGO/charity trust</a></li>

              <li><a href="ref_waqftrust.php">Waqf trust</a></li>

              <li><a href="ref_madrsatrust.php">Madrsa trust</a></li>

              <li><a href="ref_kabrastantrust.php">kabrastan trust</a></li>

              <li><a href="ref_inam_waqf.php">Enam waqf</a></li>

              <li><a href="ref_MDC.php">MDC</a></li>

              <li><a href="ref_CDC.php">CDC</a></li>

              <li><a href="ref_MRC.php">MRC</a></li>

              <li><a href="ref_technobuiz.php">Technobuiz</a></li>

              <li><a href="ref_tijarat.php">Tijarat</a></li>

            </ul>

          </li>

          <li><a href='helping_hands.php'>Helping Hands</a></li>

          <li class=''><a href='#'>Contact</a>

            <ul>

              <li><a href="contact_us.php">Contact</a></li>

              <li><a href="feedback.php">Feedback</a></li>

            </ul>

          </li>

        </ul>

      </div>

    </div>

  </div>

</header>

<div class="Modern-Slider">

  <!-- Slide 1 -->

  <div class="item">

    <div class="img-fill"> <img src="images/WhatsApp Image 2022-02-16 at 5.57.52 PM (3).jpeg"alt="">

      <div class="info">

        <div >

          <h3>Maharashtra Minority Ngo <em>Forum</em></h3>

        </div>

      </div>

    </div>

  </div>

  <!-- // Slide 1 -->

  <!-- Slide 2 -->

  <div class="item">

    <div class="img-fill"> <img src="images/WhatsApp Image 2022-02-16 at 5.57.52 PM (2).jpeg" alt="">

      <div class="info">

        <div>

          <h3>Maharashtra Minority Ngo <em>Forum</em></h3>

        </div>

      </div>

    </div>

  </div>

  <!-- // Slide 2 -->

  <!-- Slide 3 -->

  <div class="item">

    <div class="img-fill"> <img src="images/WhatsApp Image 2022-02-16 at 5.57.52 PM (3).jpeg" alt="">

      <div class="info">

        <div>

          <h3>Maharashtra Minority Ngo <em>Forum</em></h3>

        </div>

      </div>

    </div>

  </div>

  <!-- // Slide 3 -->

  <!-- Slide 4 -->

  <div class="item">

    <div class="img-fill"> <img src="images/WhatsApp Image 2022-02-16 at 5.57.52 PM.jpeg" alt="">

      <div class="info">

        <div>

          <h3>Maharashtra Minority Ngo <em>Forum</em></h3>

        </div>

      </div>

    </div>

  </div>

  <!-- // Slide 4 -->

</div>

<section class="top-slider-features wow fadeIn" data-wow-duration="1.5s">

  <div class="container">

    <div class="row">

      <div class="col-md-12">

        <div class="slider-top-features">

          

          

          <div id="owl-top-features" class="owl-carousel owl-theme">

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/mdc.jpg"  alt=""></a> </div>

              <div class="down-content"> <a href="dept_mdc.php">Minority Development Centre (MDC) </a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/cdc.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_cdc.php">Community Development Centre (CDC)</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/dpc.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_dpc.php">Data and Planning Centre (DPC)</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/marti.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_marti.php">Minority Affairs Research & training Institute (MARTI) </a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/ace.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_ace.php">Academy for Competitive Exam (ACE)</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/human.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_hrnl.php">Human Rights and Legal Forum</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/skill.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_sdf.php">Skill Developement Forum</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/technobuiz.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_technobuiz.php">Technobuiz</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/bd.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_bdf.php">Business Development Forum</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/htpc.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_healthcare.php">Healthcare and Paramedical Training Centre</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/women.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_women_emp.php">Women Empowerment Centre</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/fdc.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_financial_dev.php">Financial Development Centre</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/toor.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_travel.php">Travel and Guidance Centre</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/flood.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_disaster_mg.php">Disaster Management Centre</a> </div>

            </div>

            <div class="item car-item">

              <div class="thumb-content"> <a href="#"><img src="images/small banner/wakf.jpg" alt=""></a> </div>

              <div class="down-content"> <a href="dept_waqi_dev.php">Waqf Development Forum </a> </div>

            </div>

          </div>

        </div>

      </div>

    </div>

  </div>

</section>



<section>

  <div class="testimonials">

    <div class="container"> 

    

     <div class="row">

          <div class="col-md-4"> 

           <img src="assets/images/sponcer.png" class="img-fluid"> 

          </div>

          

     <div class="col-md-8">

     <br>

      <br>

      <h3 align="center">Our Sponsors</h3>

      <div id="owl-testimonials" class="owl-carousel owl-theme">

        

       <?php

				$sql = 'SELECT * FROM sponsors  ORDER BY id DESC';  

				$result=$link->query($sql);

                while($row = $result->fetch_array()){  ?>

                <?php $id = $row['id'];?>

        <div class="itemm wow fadeIn" data-wow-duration="0.75s">

          <div class="thumb-content">

            <div class="car-banner"> </div>

            <div class="thumb-inner"> <img src="admin/upload/sponsors/<?php echo $row['upload'];?>"> </div>

          </div>

        </div>

        

         <?php

				}

				?>

        

        

         

         

      </div>

     

          </div>

          

          

          </div>

          

    

    

    

    </div>

  </div>

</section>





<section>

  <div class="call-to-action wow fadeIn" data-wow-duration="0.75s">

    <div class="container">

      <div class="call-to-action-content">

        <div class="row">

          <div class="col-md-12">

            <h3 align="center" style="color:#FFFFFF">About Us</h3>

            <p align="justify"><small>Maharashtra Minority NGO Forum's story is linked with Jakir Shikalgar's life story and his engagement with social work. It all began in Sept. 2018 when he was doing his social work on individual level. He understood that if we really wish to work on revolutionary level, we all need to come together on a common point basis. He formed various WhatsApp groups and started sharing the issues untouched by our NGOs; not only he just posted issues but also tried to give better solutions. This lead him to form a Forum viz; MMNF. Within a short span, it spread throughout the state. This brainchild idea of creating a Forum connected more than 300 NGOS from 31 Districts of Maharashtra State. Alhamdulillah MMNF is now a recognized state wise organization which is dedicated and devoted to work for the betterment of the community through its various projects viz; Minority Development Centre (MDC). Community Development Centre (CDC), Centre for Social & Educational Research, Human Rights & Legal issues, Business Paramedical & Skill Development, Women Development Centre (WDC) & Finance Development Centre (FDC) Through all these projects. MMNF is committed to connect and co-relate all NGOS from all corners of the state to empower them & to empower the community at large. 

              MMNF creates a state level federation of Minority NGO’s, as well as create a bunch of expert persons, which called as Knowledge Tank with the help of Knowledge tank, MMNF always trying to give solutions to community in education, health, Industry, business, skills, legal, financial etc. to up-liftment of community in all manner. </small></p>

          </div>

        </div>

      </div>

    </div>

  </div>

</section>

<section>

  <div class="features-search-section">

    <div class="container">

      <div class="row">

        <div class="col-md-9">

        

        

        

        

        

        

        

        

        

         

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

        

          <div class="row">

            <div class="col-md-6">

              <div class="service-item wow fadeIn" data-wow-duration="0.75s"> <i class="fa fa-rocket"></i>

                <!--<img src="assets/images/stor_bg.jpg">-->

                <div class="text-content" align="justify">

                  <h6>Mission</h6>

                  <p>Empowerment to NGO’s with state level platform of various sectors for Up-liftment of community in all manner and Work for Rights of Minorities in Educate & train Community via Minority Development Centre & Community development centre.</p><br><br>

                </div>

              </div>

            </div>

            <div class="col-md-6" >

              <div class="service-item wow fadeIn" data-wow-duration="0.75s" > <i class="fa fa-eye"></i>

                <div class="text-content" align="justify">

                  <h6>Vision</h6>

                  <p>Design social network & strategic Organization to serve up to grass root level.To aware, inform, train to social activist & NGO’s to use government/non-government Schemes and Make a Research of all issue’s related to backwardness of minorities & demand to government/non-government organization with proper systematic way.</p>

                </div>

              </div>

            </div>

            <div class="col-md-6">

              <div class="service-item wow fadeIn" data-wow-duration="0.75s"> <i class="fa fa-users"></i>

                <div class="text-content" align="justify">

                  <h6>Goals</h6>

                 <p>MMNF is state level minority NGO forum, will connect more than 500 Active NGO’s in Maharashtra state & 3000 on national level. Also connecting 20,00,000 beneficiary with the help of NGO’s network connected with MMNF.</p><br>

                </div>

              </div>

            </div>

            <div class="col-md-6">

              <div class="service-item wow fadeIn" data-wow-duration="0.75s"> <i class="fa fa-book"></i>

                <div class="text-content" align="justify">

                  <h6>Objectives</h6>

                  <p>To achieve goals in up-liftment of community, making a three/four tier organization which parallel associate central government, state government, or NGO’s & social respondent who work in nation building. To create MDC, CDC, MRC, various facilities under one roof at each city level/village level.</p> 

                </div>

              </div>

            </div>

          </div>

        </div>

        <div class="col-md-3">

          <div class="wow fadeIn" data-wow-duration="0.75s"> 

             <img src="images/sideimg.jpg" class="img-fill" style="margin-top:50px;">

             

          </div>

        </div>

      </div>

    </div>

  </div>

  </div>

</section>









<section>

  <div class="features-search-section">

    <div class="container">

      <div class="row">

         

        <div class="col-md-12">

          <div class="search-content wow fadeIn" data-wow-duration="0.75s" style="padding:5px;">

            <div class="search-heading">

              <div class=""> <i class=""></i> </div>

              <br>

              <div class="text-content">

                <h2>Notice Board</h2>

              </div>

            </div>

            <div class="search-form">

              <table width="100%" border="1" class="table table-hover " style="color:#FFFFFF">

                <thead>

                  <tr>

                    <th>Sr.</th>

                    <th>Date</th>

                    <th>Title</th>

                    <th>Description</th>

                  </tr>

                </thead>

                <tbody>

                  <?php

				$sql = 'SELECT * FROM notice_board   ORDER BY id DESC';  

				$result=$link->query($sql);

                while($row = $result->fetch_array()){  ?>

                  <?php $id = $row['id'];?>

                  <tr>

                    <td><a href="admin/upload/notice_board/<?php echo $row['upload'];?>" style="color:#FFFFFF"><?php echo $row['id'];?></a></td>

                    <td><a href="admin/upload/notice_board/<?php echo $row['upload'];?>" style="color:#FFFFFF"><?php echo $row['datee'];?></a></td>

                    <td><a href="admin/upload/notice_board/<?php echo $row['upload'];?>" style="color:#FFFFFF"><?php echo $row['title'];?></a></td>

                    <td><a href="admin/upload/notice_board/<?php echo $row['upload'];?>" style="color:#FFFFFF"><?php echo $row['description'];?></a></td>

                    <?php

				}

				?>

                </tbody>

              </table>

              <div class="row">

                <div class="col-md-12"> </div>

              </div>

            </div>

          </div>

        </div>

      </div>

    </div>

  </div>

  </div>

</section>







<hr>

<section>

  <div class="recent-cars">

    <div class="container">

      <div class="recent-car-content">

        <div class="row">

          <div class="col-md-12">

            <div class="section-heading">

              <!--<div class="icon"> <img src="ngo1.jpg"></a> </div>-->

              <div class="text-content">

                <h2>Photos & Video Gallery</h2>

              </div>

            </div>

          </div>

        </div>

        

        

        

        <div class="row"> 

        <div class="col-md-12" align="right"> 

        <a href="photogallery.php" style="color:#666666"><u>View All Photos</u></a>

        </div>

        

        

        <?php

				$sql = 'SELECT * FROM photogallery  ORDER BY id DESC LIMIT 3';  

				$result=$link->query($sql);

                while($row = $result->fetch_array()){  ?>

          <div class="col-md-4 col-sm-6">

            <div class="car-item wow fadeIn" data-wow-duration="0.75s">

              <div class="thumb-content">

                <div class="car-banner"> </div>

                <div class="thumb-inner"> <img src="admin/upload/photogallery/<?php echo $row['upload'];?>" alt=""> </div>

              </div>

              <div class="down-content"> <a href="#">

                <h4><?php echo $row['title'];?></h4>

                </a>

                <!--<p>Leggings edison bulb hexagon, hashtag coloring book ethical echo park austin fam succulents.</p>-->

              </div>

            </div>

          </div>

          

          <?php } ?>

          

         <!-- <div class="col-md-4 col-sm-6">

            <div class="car-item wow fadeIn" data-wow-duration="0.75s">

              <div class="thumb-content">

                <div class="car-banner"> </div>

                <div class="thumb-inner"> <img src="images/mdc.jpg" alt=""> </div>

              </div>

              <div class="down-content"> <a href="#">

                <h4>Awareness program about Minority Development at Ajra Dist. Kolhapur </h4>

                </a>

                <p>Leggings edison bulb hexagon, hashtag coloring book ethical echo park austin fam succulents.</p>

              </div>

            </div>

          </div>

          <div class="col-md-4 col-sm-6">

            <div class="car-item wow fadeIn" data-wow-duration="0.75s">

              <div class="thumb-content">

                <div class="car-banner"> </div>

                <div class="thumb-inner"> <img src="images/statelevel.jpg" alt=""></a> </div>

              </div>

              <div class="down-content"> <a href="#">

                <h4>3<sup>rd</sup> State Level NGO Conference at Aurangabad</h4>

                </a>

                <p>Leggings edison bulb hexagon, hashtag coloring book ethical echo park austin fam succulents.</p>

              </div>

            </div>

          </div>

          <div class="col-md-4 col-sm-6">

            <div class="car-item wow fadeIn" data-wow-duration="0.75s">

              <div class="thumb-content">

                <div class="car-banner"> </div>

                <div class="thumb-inner"> <img src="images/social work.jpeg" height="235px" alt=""></a> </div>

              </div>

              <div class="down-content">

                <h4>1<sup>st</sup> regional Ngo conference at kalyan (Thane)</h4>

                </a>

                <p>Leggings edison bulb hexagon, hashtag coloring book ethical echo park austin fam succulents.</p>

              </div>

            </div>

          </div>-->

          

          </div>

          <hr>

          <div class="row">

          <div class="col-md-12" align="right"> 

        <a href="videogallery.php" style="color:#666666"><u>  All Other Videos</u></a>

        </div>

         

        <div class="col-md-4 col-4">

					  	<div class="item">

					 		<div class="thumb-content">

								<div class="date-post">

									<a href="#"></a>

								</div>

								<div class="thumb-inner">	<a href="https://youtube.com/channel/UCg1EolHKcQml1GdKHkjXPqQ"><img src="images/youtube1.jpg"  class="img-fluid"></a></div>

							</div> 

					  	</div>

                        </div> 

        

       

          	<div class="col-md-4 col-4">

					  	<div class="item">

					 		<div class="thumb-content">

								<div class="date-post">

									<a href="#"></a>

								</div>

								<div class="thumb-inner">

										<a href="https://youtube.com/channel/UCtI2VoL7BxiXUsWAOyhWLww"><img src="images/youtube2.jpg" class="img-fluid" ></a>

								</div>

							</div> 

					  	</div>

                        </div>

                        

                        	<div class="col-md-4 col-4">

					  	<div class="item">

					 		<div class="thumb-content">

								<div class="date-post">

									<a href="#"></a>

								</div>

								<div class="thumb-inner">

									<a href="https://youtube.com/channel/UCk8Yw2nt2LJLqiuBrQ9Otqw"><img src="images/youtube3.jpg" class="img-fluid" ></a>

								</div>

							</div> 

					  	</div>

                        </div>

            <!-- <div class="col-md-4 col-4">

					  	<div class="item">

					 		<div class="thumb-content">

								<div class="date-post">

									<a href="#"></a>

								</div>

								<div class="thumb-inner">

									<a href="#"><iframe class="embed-responsive" src="https://www.youtube.com/embed/4KI6AGMBInM" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></a>

								</div>

							</div>

							<div class="down-content">

								<a href="#"><h6>Maharashtra minority ngo</h6></a>

								

							</div>

					  	</div>

                        </div>       

            <div class="col-md-4 col-4">

					  	<div class="item">

					 		<div class="thumb-content">

								<div class="date-post">

									<a href="#"></a>

								</div>

								<div class="thumb-inner">

									<a href="#"><iframe class="embed-responsive" src="https://www.youtube.com/embed/eW4n-Lbz7bc" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></a>

								</div>

							</div>

							<div class="down-content">

								<a href="#"><h6>Efforts made to solve the problems of minorities</h6></a>

								

							</div>

					  	</div>

                        </div>-->

        </div>

        <hr>

      </div>

    </div>

  </div>

</section>







<section>

  <div class="our-clients">

    <div class="container">

      <div class="row">

        <div class="col-md-12">

          <div class="section-heading">

            <div class="text-content">

              <h2><strong>NGO’S & ORGANIZATIONS WITH US</strong></h2>

            </div>

          </div>

        </div>

      </div>

      <div class="row">

        <div class="col-md-12">

          <div id="sponcers" class="owl-carousel owl-theme owl-loaded owl-drag">

            <div class="owl-stage-outer">

              <div class="owl-stage" style="width: 3761px; transform: translate3d(-1410px, 0px, 0px); transition: all 0.25s ease 0s;">

                

                 <?php

				$sql = 'SELECT * FROM advertisements   ORDER BY id DESC';  

				$result=$link->query($sql);

                while($row = $result->fetch_array()){  ?>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="admin/upload/advertisements/<?php echo $row['upload'];?>" alt=""> </div>

                </div>

                <?php }?>

                

                <!--<div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (7).jpg" alt=""> </div>

                </div>

                

                

                

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (8).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (9).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (10).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (11).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (12).jpg" alt=""> </div>

                </div>

                <div class="owl-item" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (1).jpg" alt=""> </div>

                </div>

                <div class="owl-item" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (2).jpg" alt=""> </div>

                </div>

                <div class="owl-item" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (3).jpg" alt=""> </div>

                </div>

                <div class="owl-item active" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (4).jpg" alt=""> </div>

                </div>

                <div class="owl-item active" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (5).jpg" alt=""> </div>

                </div>

                <div class="owl-item active" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (6).jpg" alt=""> </div>

                </div>

                <div class="owl-item active" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (7).jpg" alt=""> </div>

                </div>

                <div class="owl-item active" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (8).jpg" alt=""> </div>

                </div>

                <div class="owl-item active" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (9).jpg" alt=""> </div>

                </div>

                <div class="owl-item" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (10).jpg" alt=""> </div>

                </div>

                <div class="owl-item" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (11).jpg" alt=""> </div>

                </div>

                <div class="owl-item" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (12).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (1).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (2).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (3).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (4).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (5).jpg" alt=""> </div>

                </div>

                <div class="owl-item cloned" style="width: 146.667px; margin-right: 10px;">

                  <div class="item"> <img src="assets/images/Cilents &amp; Partners/clinets (6).jpg" alt=""> </div>

                </div>-->

              </div>

            </div>

            <div class="owl-nav">

              <button type="button" role="presentation" class="owl-prev"><span aria-label="Previous">‹</span></button>

              <button type="button" role="presentation" class="owl-next"><span aria-label="Next">›</span></button>

            </div>

           <!--  <div class="owl-dots">

             <button role="button" class="owl-dot active"><span></span></button> 

              <button role="button" class="owl-dot"><span></span></button>

            </div>-->

          </div>

        </div>

      </div>

    </div>

  </div>

</section>

<footer>

  <div class="container">

    <div class="row">

      <div class="col-md-3">

        <div class="footer-item">

          <div class="what-offer">

            <h2>Departments</h2>

            <ul>

              <li style="margin-bottom:5px;"><a href="dept_mdc.php" style="padding:0px;">Minority Development Centre (MDC) </a></li>

              <li style="margin-bottom:5px;"><a href="dept_cdc.php" style="padding:0px;">Community Development Centre (CDC)</a></li>

              <li style="margin-bottom:5px;"><a href="dept_dpc.php" style="padding:0px;">Data and Planning Centre (DPC)</a></li>

              <li style="margin-bottom:5px;"><a href="dept_marti.php" style="padding:0px;">Minority Affairs Research & training Institute (MARTI) </a></li>

              <li style="margin-bottom:5px;"><a href="dept_ace.php" style="padding:0px;">Academy for Competitive Exam (ACE)</a></li>

            </ul>

          </div>

        </div>

      </div>

      <div class="col-md-3">

        <div class="footer-item">

          <div class="what-offer">

            <h2>&nbsp;</h2>

            <ul>

              <li style="margin-bottom:5px;"><a href="dept_hrnl.php"  style="padding:0px;">Human Rights and Legal Forum</a></li>

              <li style="margin-bottom:5px;"><a href="dept_sdf.php" style="padding:0px;">Skill Developement Forum</a></li>

              <li style="margin-bottom:5px;"><a href="dept_technobuiz.php" style="padding:0px;">Technobuiz-Business School, Technical, Educational and Research Development Centre</a></li>

              <li style="margin-bottom:5px;"><a href="dept_bdf.php" style="padding:0px;">Business Development Forum</a></li>

              <li style="margin-bottom:5px;"><a href="dept_healthcare.php" style="padding:0px;">Healthcare and Paramedical Training Centre</a></li>

            </ul>

          </div>

        </div>

      </div>

      <div class="col-md-3">

        <div class="footer-item">

          <div class="need-help">

            <h2>&nbsp;</h2>

            <ul>

              <li style="margin-bottom:5px;"><a href="dept_women_emp.php" style="padding:0px;">Women Empowerment Centre</a></li>

              <li style="margin-bottom:5px;"><a href="dept_financial_dev.php" style="padding:0px;">Financial Development Centre</a></li>

              <li style="margin-bottom:5px;"><a href="dept_travel.php" style="padding:0px;">Travel and Guidance Centre</a></li>

              <li style="margin-bottom:5px;"><a href="dept_disaster_mg.php" style="padding:0px;">Disaster Management Centre</a></li>

              <li style="margin-bottom:5px;"><a href="dept_waqi_dev.php" style="padding:0px;">Waqf Development Forum </a></li>

            </ul>

          </div>

        </div>

      </div>

      <div class="col-md-3">

        <div class="footer-item">

          <div class="quick-search">

            <h2>Quick Search</h2>



            <ul>  

              <li style="margin-bottom:5px;"><a href="terms_conditions.php" style="padding:0px;">Terms and Conditions </a></li>

              <li style="margin-bottom:5px;"><a href="privacy-policy.php" style="padding:0px;">Privacy-Policy </a></li>

              <li style="margin-bottom:5px;"><a href="shipment-policy.php" style="padding:0px;">Shipment Policy</a></li>

              <li style="margin-bottom:5px;"><a href="refund_cancellation-policy.php" style="padding:0px;">Refund/Cancellation Policy</a></li>

              <li style="margin-bottom:5px;"><a href="contact_us.php" style="padding:0px;">Contact Us</a></li>

            </ul>

          </div>

        </div>

      </div>

      <div class="col-md-12">

        <div class="sub-footer">

          <p>Copyright 2022-23. All rights reserved by: <a href="http://www.hudastechnologies.com/">Huda's Technologies</a></p>

        </div>

      </div>

    </div>

  </div>

</footer>

<script src="assets/js/jquery-1.11.0.min.js"></script>

<!-- Slider Pro Js -->

<script src="assets/js/sliderpro.min.js"></script>

<!-- Slick Slider Js -->

<script src="assets/js/slick.js"></script>

<!-- Owl Carousel Js -->

<script src="assets/js/owl.carousel.min.js"></script>

<!-- Boostrap Js -->

<script src="assets/js/bootstrap.min.js"></script>

<!-- Boostrap Js -->

<script src="assets/js/wow.animation.js"></script>

<!-- Custom Js -->

<script src="assets/js/custom.js"></script>

</body>

</html>

